...
SELECT
    ...
    report_dashboard.creator_id AS owner_id,
    ...
    CONCAT('https://domain.metabase.net/dashboard/', report_dashboard.id) AS dashboard_hyperlink,
    CONCAT('https://domain.metabase.net/collection/', report_dashboard.collection_id) AS collection_hyperlink
FROM report_dashboard
    LEFT JOIN core_user
        ON report_dashboard.creator_id = core_user.id
    LEFT JOIN collection
        ON report_dashboard.collection_id = collection.id
    LEFT JOIN (
        SELECT
            model_id,
            COUNT(*) AS view_count,
...
ORDER BY COALESCE(view_log.view_count, 0), report_dashboard.id
;
